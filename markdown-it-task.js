    ...
    state.tokens.forEach((token, index) => {
      if (token.type === 'inline' &&
        ...
        if (state.tokens[index - 2].attrIndex('class') < 0) {
          state.tokens[index - 2].attrPush(['class', options])
        } else {
          state.tokens[index - 2].attrs[index] = ['class', options]
        }
      }
    })
  })
